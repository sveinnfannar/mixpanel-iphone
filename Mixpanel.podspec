Pod::Spec.new do |s|
  s.name         =  'Mixpanel'
  s.version      =  '2.0.2'
  s.license      =  'Apache License'
  s.summary      =  'A fork of the iPhone tracking library for Mixpanel Analytics.'
  s.homepage     =  'http://mixpanel.com'
  s.author       =  { 'Mixpanel' => 'support@mixpanel.com' }
  s.source       =  { :git => 'https://github.com/sveinnfannar/mixpanel-iphone.git', :tag => "v#{s.version}" }
  s.frameworks   =  'UIKit', 'Foundation', 'SystemConfiguration', 'CoreTelephony', 'AdSupport'
  s.platform     =  :ios, '4.3'
  s.source_files =  'Mixpanel/**/*.{m,h}'
  s.private_header_files =  'Mixpanel/Library/**/*.h'
  s.requires_arc =  false
  spec.ios.resource_bundle = { 'Carriers' => 'Mixpanel/Carriers.plist' }
end
